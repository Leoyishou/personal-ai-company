#!/usr/bin/env node
/**
 * 产品事业部 - TestFlight 部署上报
 *
 * 触发时机：PostToolUse (Skill: api-deploy-testflight)
 * 职责：检测 TestFlight 部署成功 → 上报 PMO
 */
...
function main(hookData) {
  ...
  // 只处理 Skill 工具且是 api-deploy-testflight
  if (tool_name !== 'Skill') return;
  const skillName = tool_input?.skill || tool_input?.name || '';
  if (!skillName.includes('testflight') && !skillName.includes('deploy-testflight')) return;
  ...
  if (result.result === 'created') {
    console.log(`✅ PMO: TestFlight deploy tracked - ${result.issue?.identifier}`);
  }
}
