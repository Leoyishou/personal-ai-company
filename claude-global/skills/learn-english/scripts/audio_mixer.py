...
def cut_audio(
    ...
    Args:
        ...
        end_ms: 结束时间（毫秒）
        normalize: 是否标准化格式
        trailing_silence_ms: 在末尾追加的静音时长（毫秒），防止截断
    ...
    if normalize:
        if trailing_silence_ms > 0:
            silence_sec = trailing_silence_ms / 1000
            args.extend(["-af", f"apad=pad_dur={silence_sec}"])
...
def slow_down_audio(
    ...
    # apad=pad_dur=0.5 在 atempo 前补 0.5s 静音，确保 atempo 内部缓冲区被完全 flush
    # 不加的话 atempo 会在短段末尾丢失几百毫秒（WSOLA overlap-add 缓冲区未排空）
    args = [
        "-i", input_file,
        "-filter:a", f"apad=pad_dur=0.5,atempo={tempo}",
