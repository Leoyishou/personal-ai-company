...
def call_openrouter_for_explanation(sentence: str, unfamiliar_words: list[str]) -> tuple[str, list[dict]]:
    ...
    prompt = f"""你是英语口语教练。用户正在听英文音频，遇到了不认识的词。

请先逐词解释生词，再给整句翻译。要求：
1. 生词解释不超过 5 个字，贴合这句话的语境
2. 整句翻译用口语，像朋友聊天一样说
3. 只返回 JSON
...
返回格式：
{{"words": [{{"word": "xxx", "meaning": "简短解释"}}], "translation": "整句翻译"}}"""

    try:
        result = subprocess.run(
            ["python", str(OPENROUTER_SCRIPT), "--prompt", prompt, "--model", "google/gemini-2.5-pro"],
            capture_output=True,
            text=True,
            timeout=30,
...
def select_enhancement_points(
    ...
    for s in sentences:
        ...
        if unfamiliar:
            ...
            selected.append(EnhancementPoint(
...
def adjust_speed_if_needed(
    ...
    # 减速音频
    # atempo 只支持 0.5-2.0，低于 0.5 需串联两个 atempo 滤镜
    if atempo < 0.5:
        atempo_filter = f"atempo=0.5,atempo={atempo / 0.5}"
    else:
...
def enhance_audio(
    ...
    for ep in enhancement_points:
        ...
        # trailing_silence_ms=300: ASR end_ms 可能比实际语音尾音早，追加静音防止截断
        cut_audio(source_audio, str(highlight_orig), ep.start_ms, ep.end_ms, trailing_silence_ms=300)
