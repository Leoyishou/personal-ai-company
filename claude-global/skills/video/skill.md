...
| 钩子类型 | 说明 | 示例 |
|----------|------|------|
| 结果前置 | 直接展示最终效果/成品 | 先看成果，再讲过程 |
| 悬念提问 | 提出观众关心的问题 | "你知道为什么...？" |
| 反常识 | 打破认知预期 | "99%的人都做错了" |
| 图案中断 | 视觉信息流突然变化 | 快速镜头切换/强视觉对比 |

**执行**：视频第一个场景的配音文案必须是钩子，不能有寒暄/铺垫。

### 2. 好奇心闭环结构

```
主闭环（覆盖全视频）：开场提出问题 → 结尾揭示答案
├── 小闭环1（0-30s）：提出子问题 → 给出价值
├── 小闭环2（30-60s）：新悬念 → 奖励
└── 小闭环3（60-90s）：深化 → 总结
```

- 每30-45秒设置一个小闭环，给观众多巴胺奖励
- 每个小闭环关闭前，先打开下一个悬念
...
| 规则 | 执行标准 |
|------|----------|
| 视觉变化频率 | 每5秒至少1次（镜头切换/文字叠加/动画/颜色变化）|
| J-cut 剪辑 | 新场景音频提前0.5s进入，过渡更流畅 |
| 消灭死亡空间 | 任何>3秒无变化的段落必须插入B-roll/文字/动画 |
| 信息密度 | 每句话都服务核心信息，删除一切冗余 |

### 4. 字幕规范

- 每块字幕 15-20 个字符（认知最优长度）
- 关键词用大号/不同颜色强调
- 字幕与语音同步偏差 < 200ms（用 Whisper 确保精度）
- 即使有声音也要加字幕（双通道信息留存率更高）

### 5. 叙事结构选择

| 内容类型 | 推荐结构 |
|----------|----------|
| 知识科普 | 起（问题）→ 承（积累）→ 转（意外）→ 合（解答）|
| 产品对比 | 问题 → 解决方案 → 证明（数据/案例）|
| 技巧教程 | 结果前置 → 步骤拆解 → 总结复用 |
| 故事叙事 | 冲突 → 发展 → 高潮 → 结局 |
