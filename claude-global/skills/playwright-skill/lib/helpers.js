...
// Default path for user's Chrome profile copy
const USER_PROFILE_PATH = process.env.PW_USER_PROFILE ||
  require('path').join(require('os').homedir(), '.playwright-profile');
 ...
 *
 * SETUP: Run `chrome-debug` alias (defined in ~/.zshrc)
 * First time: Sign in to Google in the debug Chrome to sync your sessions
 *
 * @returns {Promise<{browser: Browser, context: BrowserContext, page: Page, close: Function}>}
 */
async function launchWithUserProfile() {
  ...
  try {
    ...
    console.log('🔐 Connected to Chrome via CDP (all login sessions shared)');
    ...
    return {
      ...
      close: async () => {
        ...
        await browser.close();
        console.log('📤 Disconnected from Chrome (browser still running)');
      }
    };
  } catch (e) {
    if (e.message.includes('ECONNREFUSED') || e.message.includes('connect')) {
      ...
      console.error('');
      console.error('首次使用需在该 Chrome 中登录 Google 账号以同步会话');
    }
    throw e;
  }
}
