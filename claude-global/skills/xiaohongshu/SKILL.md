...
1. **REST API（推荐）**：通过 `curl` 调用 `http://localhost:18060/api/v1/publish`
2. **MCP 工具（备选）**：调用 `publish_content` / `publish_with_video`

> **CRITICAL: 禁止重复发布！每次发布请求只能调用一次。**
> - 禁止在同一次用户请求中多次发布
> - 如果发布已成功，不要再次尝试发布

**为什么优先 REST API？**
- MCP 工具在 Claude Code 终端中无法将返回的图片保存到本地文件（如二维码）
- REST API 可以直接用 curl 获取数据并用 Python 处理
- REST API 和 MCP 工具调用同一个后端服务，功能完全一致
...
**IMPORTANT: 登录必须使用 REST API，不要用 MCP 工具获取二维码。**

MCP 工具返回的二维码图片数据无法在 Claude Code 终端中保存为本地文件，导致用户无法扫码。REST API 可以完美解决这个问题。
...
| `/api/v1/login/qrcode` | GET | 获取登录二维码（返回 Base64） |
| `/api/v1/login/cookies` | DELETE | 删除 cookies，重置登录 |
...
# 返回: {"success":true,"data":{"is_logged_in":true/false,...},...}
```

**Step 2: 如果未登录，获取二维码并保存到本地文件**
```bash
curl -s http://localhost:18060/api/v1/login/qrcode | python3 -c "
import sys, json, base64
resp = json.load(sys.stdin)
img_data = resp['data']['img']
b64 = img_data.split(',')[1] if ',' in img_data else img_data
with open('/Users/liuyishou/tmp/xiaohongshu_qrcode.png', 'wb') as f:
    f.write(base64.b64decode(b64))
print('Saved to /Users/liuyishou/tmp/xiaohongshu_qrcode.png')
"
```
...
```
请用小红书 App 扫码登录
二维码图片：/Users/liuyishou/tmp/xiaohongshu_qrcode.png
```

**Step 4: 用户确认扫码后，再次检查登录状态**
```bash
curl -s http://localhost:18060/api/v1/login/status
```

**注意事项：**
- 二维码保存到 `~/tmp/` 目录（用户偏好的下载目录）
- 二维码有效期约 4 分钟，超时后重新获取
- 登录信息会被保存，后续发布无需重复登录
- 如需切换账号，先 `curl -X DELETE http://localhost:18060/api/v1/login/cookies`

## 发布工作流

当用户请求发布小红书内容时，遵循以下流程：

### Step 1: 确认用户已登录（强制，使用 REST API）
...
**如果已登录（`is_logged_in: true`）：** 继续下一步

**如果未登录：** 执行 REST API 登录流程（见「3. 登录管理」章节）
1. `curl` 获取二维码 → `python3` 解码保存为 PNG
2. 告知用户二维码路径：`/Users/liuyishou/tmp/xiaohongshu_qrcode.png`
3. 用户确认扫码后，再次 `curl` 检查登录状态
4. **只有确认登录成功后，才能继续发布流程**
...
> **CRITICAL: 只能发布一次。成功后直接进入 Step 7，禁止重试。**
...
# 发布视频
curl -s -X POST http://localhost:18060/api/v1/publish_video \
  -H "Content-Type: application/json" \
  -d '{
    "title": "标题",
    "content": "正文",
...
调用 `publish_content` 或 `publish_with_video` MCP 工具，传入相同参数。
```
...
xiaohongshu-mcp 服务同时提供 MCP 和 REST API 两套接口，功能完全一致。
...
| `/health` | GET | 健康检查 |
| `/api/v1/login/status` | GET | 检查登录状态 |
| `/api/v1/login/qrcode` | GET | 获取登录二维码（返回 `data.img` 为 base64 PNG） |
| `/api/v1/login/cookies` | DELETE | 删除 cookies，重置登录 |
| `/api/v1/publish` | POST | 发布图文笔记 |
| `/api/v1/publish_video` | POST | 发布视频笔记 |
| `/api/v1/feeds/list` | GET | 获取 Feeds 列表 |
| `/api/v1/feeds/search` | GET/POST | 搜索 Feeds（`keyword` 参数） |
| `/api/v1/feeds/detail` | POST | 获取 Feed 详情（`feed_id`, `xsec_token`） |
| `/api/v1/user/profile` | POST | 获取用户主页 |
| `/api/v1/user/me` | GET | 获取当前登录用户信息 |
