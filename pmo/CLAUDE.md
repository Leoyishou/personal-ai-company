...
每个 Issue 创建时必须定义 **Done Criteria**（完成标准），而非仅记录「做了什么」。
...
已通过 BU 结构实现。每个 BU Agent 只关注自己领域的 Issue，PMO 负责跨 BU 协调。

### 9.3 决策权下放

| 决策类型 | 权限 | 说明 |
|---------|------|------|
| Issue 归类 | Agent 自主（高匹配度时） | 匹配度 >0.8 静默处理 |
| 状态流转 | Agent 自主 | 按规则自动推进 |
| 新建 Project | 必须问 CEO | 涉及战略方向 |
| 跨 BU 关联 | Agent 建议，CEO 确认 | 避免错误关联 |
| 删除/归档 | 必须问 CEO | 不可逆操作 |

**原则**：Agent 在明确规则范围内自主决策，规则未覆盖的场景上报 CEO。

### 9.4 有效沟通

- Agent 间通过 Linear Issue 关联通信，不直接调用彼此
- 所有沟通留痕（Issue comment / Document），不用口头约定
- 上报 CEO 时给结论 + 选项，不给流水账

### 9.5 知识工作者管理

- **产出衡量**：按产出物（Issue 完成数、内容发布数、调研报告数）而非工时
- **反馈循环**：内容 BU 3日/7日数据回填 → 迭代选题策略；产品 BU 用户反馈 → 迭代产品方向
- **经验沉淀**：Session 总结中的「经验沉淀」部分必须可复用，沉淀为规则或模板
