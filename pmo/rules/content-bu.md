...
| 策划 | `c6bc346c-6b50-4a35-a49e-ce902c1cc59f` | 内容处于策划阶段，已制作但未决定是否发布 |
...
- **策划**：内容已制作（图 + 文案），但尚未决定是否/何时发布
- 策划阶段的内容不一定最终会发布（可能废弃、改方向、留存备用）
- 批量制作场景（如一次做 3 篇），整批完成后统一打「策划」
- 决定发布时移除「策划」，加上「发布」
...
| 低/无 | 检测是否为新系列模式（见 5.4），否则让用户选择 |

### 5.4 新系列检测

当匹配度低/无时，PMO 应检测是否出现了新的内容系列模式：

**新系列信号**：
- Session 产出 >= 2 篇同主题内容（批量制作）
- 内容有明确的系列名称（如「知乎高赞」「Reddit 热帖」）
- 素材来源统一（同一平台/同一类型）

**处理流程**：
1. 识别到新系列信号 → 建议创建新 Project
2. 用 AskUserQuestion 确认：「检测到新系列 [xxx]，是否创建为独立项目？」
3. 用户确认后：Linear 创建 Project + 更新 projects.json + 更新本规则文件
4. 用户拒绝 → 归入「随机探索系列」
...
| 14 | 知乎高赞系列 | `ad9a0ea3-a156-4f80-b37c-840f5e50e204` | 知乎, 高赞, zhihu, 万赞 | 知乎高赞回答改编为小红书图文 |
